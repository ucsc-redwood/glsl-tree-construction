...
pub fn partial_sum(in_data: Vec<u32>, n: i32, out_data: &mut Vec<u32>) {
    ...
    mod cs {
        vulkano_shaders::shader! {
            ty: "compute",
            path: "shader/simple_radixsort.comp",
            spirv_version: "1.3",
        }
    }
    ...
    let in_data_buffer = Buffer::from_iter(
    ...
    let out_data_buffer = Buffer::from_iter(
        memory_allocator.clone(),
        BufferCreateInfo {
            usage: BufferUsage::STORAGE_BUFFER,
            ..Default::default()
    ...
    let set = PersistentDescriptorSet::new(
        ...
        layout.clone(),
        [
            WriteDescriptorSet::buffer(0, in_data_buffer.clone()),
            WriteDescriptorSet::buffer(1, n_buffer.clone()),
            WriteDescriptorSet::buffer(2, out_data_buffer.clone()),
        ],
        [],
    )
    .unwrap();
    ...
    let out_data_content = out_data_buffer.read().unwrap();
    for (i, val) in out_data_content.iter().enumerate() {
        out_data[i] = *val;
    }
}
